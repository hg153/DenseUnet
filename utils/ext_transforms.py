...
class ExtRandomRotation(object):
    ...
    def __call__(self, img, lbl):
        ...
        return F.rotate(img, angle, self.resample, self.expand, self.center), F.rotate(lbl[None,:,:], angle, self.resample, self.expand, self.center).squeeze(0)
...
class ExtRandomCrop(object):
    ...
    def __call__(self, img, lbl):
        ...
        assert img.size[0] == lbl.shape[1] and img.size[1] == lbl.shape[0], 'size of img and lbl should be the same. %s, %s'%(img.size, lbl.size)
        if self.padding > 0:
